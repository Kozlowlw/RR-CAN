...
int getTurnSigData() {
  ...
  if (digitalRead(left_sig) == LOW)
    direction++;
  else if (digitalRead(right_sig) == LOW)
    direction--;
...
void sendSensorData() {
  char data[1];
  data[0] = getTurnSigData();
  bool success = sendmsg(steeringID, data, 1);
  data[0] = getLightsData();
  success = success && sendmsg(lightsID, data, 1);
  if (true) {
    if(success) {
      Serial.println("Sent driver data successfully");
    } else {
      Serial.println("Error in sending driver data...");
    }
  }
}
...
void receivemsg() {
  ...
  if(!digitalRead(CAN0_INT)) {           // If CAN0_INT pin is low, read receive buffer
    ...
    if((rxId && 0xFFFF) == lightsID || (rxId && 0xFFFF) == steeringID)
      ID_CONFLICT = true;
